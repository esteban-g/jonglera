...
import { IonicPage, NavController, NavParams, PopoverController, App, ModalController, Tabs, Platform, ActionSheetController, FabContainer } from 'ionic-angular';
...
import { PopoverspeechPage } from '../popoverspeech/popoverspeech';
/**
...
export class StartPage {
  ...
  picToViewX: string = "../assets/img/c4.png";
  ...
  startListening() {
    ...
    this.speechRecognition.startListening().subscribe(matches => {
      if(matches.length >0){
        if(matches[0].localeCompare('help')==0){
          this.presentPopoverSpeech();
        }
  ...
  takePicture() {
    this.camera.getPicture(this.options).then((imageData) => {
      ...
      console.log("takepicture ok:" + base64Image.length);
    }, (err) => {
  ...
  presentPopover() {
    ...
    const popo = this.popoverCtrl.create(PopoverPage, { d: "d" }, { cssClass: 'backdropOpacityPopover' });
    let ev = {
      target: {
    ...
    //const popo = this.popoverCtrl.create(PopoverPage, {d:"d"}, { cssClass: 'backdropOpacityPopover talk-bubble tri-right border round btm-left-in'});
    popo.present({ ev });
  ...
  presentPopoverSpeech() {

    const popo = this.popoverCtrl.create(PopoverspeechPage, { d: "d" }, { cssClass: 'backdropOpacityPopover' });
    let ev = {
      target: {
        getBoundingClientRect: () => {
          return {
